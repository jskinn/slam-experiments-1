...
def get_on_the_spot_roll_trajectory():
    ...
    return {time: tf.Transform(rotation=(time * angular_vel - np.pi / 2, 0, 0))
            for time in np.arange(0, np.pi / angular_vel, 1 / framerate)}


def get_on_the_spot_pitch_trajectory():
    """
    Get a trajectory that turns a circle on the spot. Pure pitch, no translational component.
    :return:
    """
    framerate = 30  # frames per second
    angular_vel = np.pi / 36   # radians per second
...
def get_circle_roll_trajectory():
    """
    Get a trajectory that moves in a 1m diameter semicircle, facing outwa
    :return:
    """
    framerate = 30            # frames per second
    angular_vel = np.pi / 36  # radians per second
    relative_pose = tf.Transform(location=(0, 0, 0.5), rotation=(0, 0, 0))
    return {time: (tf.Transform(rotation=(time * angular_vel - np.pi / 2, 0, 0))).find_independent(relative_pose)
            for time in np.arange(0, np.pi / angular_vel, 1 / framerate)}
...
def get_line_roll_trajectory():
    """
    Get a simple linear trajectory, moving in a straight line forwards from -10m to 10m
    and rolling through -np.pi / 2 to np.pi / 2
    :return: A map from timestamps to camera poses, with which we can create a controller.
    """
    framerate = 30  # frames per second
    speed = 1.2  # meters per second (walking pace)
    angular_vel = np.pi * speed / 20  # (pi rad * 1.2 m/s) / (20m) = (0.06 rad / s)
    return {time: tf.Transform(location=(time * speed - 10, 0, 0), rotation=(time * angular_vel - np.pi / 2, 0, 0))
            for time in np.arange(0, 20 / speed, 1 / framerate)}


def get_line_pitch_trajectory():
    """
    Get a simple linear trajectory, moving in a straight line forwards from -10m to 10m
    and pitching through np.pi / 2 to -np.pi / 2
    :return: A map from timestamps to camera poses, with which we can create a controller.
    """
    framerate = 30  # frames per second
    speed = 1.2  # meters per second (walking pace)
    angular_vel = -np.pi * speed / 20  # (pi rad * 1.2 m/s) / (20m) = (0.06 pi rad / s)
    return {time: tf.Transform(location=(time * speed - 10, 0, 0), rotation=(0, time * angular_vel + np.pi / 2, 0))
            for time in np.arange(0, 20 / speed, 1 / framerate)}
...
class TrajectoryGroup:
    ...
    def do_imports(self, task_manager: arvet.batch_analysis.task_manager.TaskManager) -> bool:
        """
